...
// src/ChatGPTPromptManager.ts
var import_obsidian3 = require("obsidian");
var Papa = __toESM(require_papaparse_min());
var ChatGPTPromptManager = class {
  constructor(plugin) {
    this.plugin = plugin;
  }
  async fetchLatestChatGPTPrompts(snippetFolderPath) {
    try {
      ...
      new import_obsidian3.Notice("Awesome ChatGPT Prompts file has been successfully saved to your snippets folder!");
    } catch (error) {
      console.error("Error fetching ChatGPT prompts:", error);
      new import_obsidian3.Notice("Failed to fetch and save the latest Awesome ChatGPT Prompts.");
    }
  }
  convertCSVToMarkdown(csvContent) {
...
`;
  ...
  async createMarkdownFile(folderPath, fileName, content) {
    ...
    if (folder && folder instanceof import_obsidian3.TFolder) {
      ...
      new import_obsidian3.Notice(`The folder path ${folderPath} does not exist or is not a valid folder.`);
...
var SnippetManagerPlugin = class extends import_obsidian4.Plugin {
  constructor() {
    super(...arguments);
    this.snippets = {};
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        if (!fileOrFolder) {
          new import_obsidian4.Notice(`Snippet path not found: ${snippetPath}`);
          return;
        }
        if (fileOrFolder instanceof import_obsidian4.TFolder) {
          ...
          new import_obsidian4.Notice(`Error: Snippet path should be an folder.`);
        }
      }
    });
  ...
  async loadSnippets() {
    ...
    } else if (fileOrFolder instanceof import_obsidian4.TFile && fileOrFolder.extension === "md") {
      await this.loadSnippetsFromFile(fileOrFolder, false);
    } else {
      new import_obsidian4.Notice(`Invalid snippet path: ${snippetPath}`);
    }
  ...
  getSnippets(content, contentCache, filePrefix) {
    ...
    for (let i = 0; i < headings.length; i++) {
      if (headings[i].level !== level) {
        new import_obsidian4.Notice(`Please follow the same heading level throughout the file`);
        return snippets;
      }
    }
    for (let i = 0; i < headings.length; i++) {
  ...
  async saveSnippetsAsAlfredJson() {
    ...
    try {
      ...
      new import_obsidian4.Notice("Failed to save snippets as Alfred JSON");
    }
  }
  async loadSettings() {
